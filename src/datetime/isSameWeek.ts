...
>(dt1: DateTime, dt2: DateTime, options: IsSameWeekOptions): boolean {
	...
	if (!Number.isInteger(firstDayOfWeek) || firstDayOfWeek < 1 || firstDayOfWeek > dt1.daysInWeek) {
		throw new Error(`${firstDayOfWeek} isn't a valid day of week`);
	}
	if (dt1.calendarId !== dt2.calendarId) {
		throw new Error(`Calendar mismatch: ${dt1.calendarId} and ${dt2.calendarId}`);
	}

	const pdt1 = toPlainDate(dt1);
	const pdt2 = toPlainDate(dt2);

	return startOfWeek(pdt1, { firstDayOfWeek }).equals(startOfWeek(pdt2, { firstDayOfWeek }));
}
