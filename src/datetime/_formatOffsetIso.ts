 ...
 */
export function formatOffsetIso(offsetSeconds: number, includeColon: boolean): string {
	const sign = offsetSeconds < 0 ? "-" : "+";
	const [h, m, s] = secondsToHms(Math.abs(offsetSeconds)).map((v) => padLeadingZeros(v, 2)) as [
		string,
		string,
		string,
	];
