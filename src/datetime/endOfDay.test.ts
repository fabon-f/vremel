...
test("ZonedDateTime and backward transition", () => {
	expect(
		...
		Temporal.ZonedDateTime.from(
			"2024-10-27T23:59:59.999999999+00:00[Europe/London]",
		),
	);
	expect(
		endOfDay(
			Temporal.ZonedDateTime.from(
				"2010-11-06T12:00:00-02:30[America/St_Johns]",
			),
		),
	).toEqual(
		Temporal.ZonedDateTime.from(
...
test("ZonedDateTime and forward transition", () => {
	expect(
		endOfDay(
			Temporal.ZonedDateTime.from("2024-03-31T00:30:00+00:00[Europe/London]"),
		),
	).toEqual(
		Temporal.ZonedDateTime.from(
			"2024-03-31T23:59:59.999999999+01:00[Europe/London]",
		),
	);
	expect(
		endOfDay(
			Temporal.ZonedDateTime.from("1919-03-30T12:00:00-05:00[America/Toronto]"),
		),
	).toEqual(
		Temporal.ZonedDateTime.from(
