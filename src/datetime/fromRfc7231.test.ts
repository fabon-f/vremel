...
test("Instant", () => {
	expect(fromRfc7231("Fri, 07 Jun 2024 01:23:45 GMT", Temporal.Instant)).toEqual(
		Temporal.Instant.from("2024-06-07T01:23:45Z"),
	);
});

test("PlainDateTime", () => {
	expect(fromRfc7231("Fri, 07 Jun 2024 01:23:45 GMT", Temporal.PlainDateTime)).toEqual(
		Temporal.PlainDateTime.from("2024-06-07T01:23:45"),
	);
});

test("ZonedDateTime", () => {
	expect(fromRfc7231("Fri, 07 Jun 2024 01:23:45 GMT", Temporal.ZonedDateTime)).toEqual(
		Temporal.ZonedDateTime.from("2024-06-07T01:23:45+00:00[UTC]"),
	);
});
...
test("leap second", () => {
	...
	expect(fromRfc7231(rfc7231, Temporal.Instant)).toEqual(Temporal.Instant.from(result));
	expect(fromRfc7231(rfc7231, Temporal.ZonedDateTime)).toEqual(Temporal.ZonedDateTime.from(result));
	expect(fromRfc7231(rfc7231, Temporal.PlainDateTime)).toEqual(Temporal.PlainDateTime.from(result));
});
...
test.for(["24:00:00", "23:58:60", "12:60:00"])("Invalid hour, minute, and second (%s)", (time) => {
...
] as [string, string][])("when year is less than 1000", ([rfc7231, iso8601]) => {
	expect(fromRfc7231(rfc7231, Temporal.Instant)).toEqual(Temporal.Instant.from(iso8601));
